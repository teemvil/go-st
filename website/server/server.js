...
client.on("message", function (topic, message) {
  ...
  let s;
  if (!devices.find((item) => item.device.hostname === m.DeviceName)) {
    try {
      let x = {
        event: m.Event,
        message: m.Message,
        messagetimestamp: m.Time,
        device: {
          itemid: m.Itemid,
          hostname: m.DeviceName,
          address: "",
          starttimestamp: m.Time,
          valid: false,
          validtimestamp: "",
        },
        sensor: {
  ...
  if (devices.find((item) => item.device.hostname === m.device.hostname)) {
    ...
    if (m.event === "device validation ok") {
      ...
      devices[s].device.valid = true;
      devices[s].device.validtimestamp = m.Time;
    }
  }
  ...
  wsServer.clients.forEach(function (item) {
    ...
    let iitemmm = `{"god": {"data": { "hostname": "${m.DeviceName}", "timestamp": "${m.Time}", "sensor": "${m.SensorName}", "message": "${m.Message}", "deviceObject": "${devices[s].device.hostname}", "valid": "${devices[s].device.valid}"}, "valdate": "${devices[s].device.validtimestamp}"}}`;
